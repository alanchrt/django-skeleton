...
try:
    ...
    if DEBUG == '':
        DEBUG = 'True'
    new_data = new_data.replace('DEBUG = None', 'DEBUG = ' + DEBUG.__str__())
...
try:
    ...
    SECRET_KEY = ''.join(random.choice(string.letters + string.digits +
                   string.punctuation.replace('\'', '')) for x in range(60))
...
try:
    ...
    new_data = new_data.replace('[[ADMIN_MEDIA_PREFIX]]', ADMIN_MEDIA_PREFIX)
...
try:
    ...
    database_backend = raw_input("Database backend ('postgresql_psycopg2', " +
...
try:
    ...
    new_data = new_data.replace('[[DB_NAME]]', DATABASES['default']['NAME'])
